...
export default function CashOrderConfirmationPage({ params }: { params: Promise<{ id: string }> }) {
        ...
        <main className="min-h-screen bg-white text-neutral-900 pt-32 pb-20">
            <div className="max-w-xl mx-auto px-6 text-center">
                ...
                <p className="text-neutral-600 text-lg mb-8">
                    Please proceed to the counter to pay <strong>${order.total.toFixed(2)}</strong> in cash.
                </p>
                ...
                <div className="bg-neutral-50 rounded-2xl p-8 mb-12 border border-neutral-200 text-left">
                    <div className="space-y-4">
                        ...
                        {/* Order Items Section - NEW */}
                        <div className="border-b border-neutral-200 pb-4">
                            <span className="text-neutral-500 uppercase text-sm tracking-wider block mb-2">Order Items</span>
                            <div className="space-y-2">
                                {order.items.map((item: any) => (
                                    <div key={item.id} className="flex justify-between text-sm">
                                        <span>{item.quantity}x {item.product.name}</span>
                                        <span className="font-mono text-neutral-600">${(item.price * item.quantity).toFixed(2)}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
